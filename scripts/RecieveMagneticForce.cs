...
public class RecieveMagneticForce : MonoBehaviour {
	...
	void Update () {
        ...
        {
            if (awaytime < Time.time)
            {
                gameObject.GetComponent<Rigidbody>().velocity = ((Player.transform.position - gameObject.transform.position).normalized) * 20.0F;
                awaytimer = 0;
            }
            Vector3 tarvec = (gameObject.transform.position - followobj.transform.position);
            gameObject.GetComponent<Rigidbody>().AddForce(tarvec.normalized*50.0F/(tarvec.magnitude* tarvec.magnitude));
        }
